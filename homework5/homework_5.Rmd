...
```{r}
questions_with_tags %>% group_by(tag_name) %>% summarise(mean_score = mean(score)) %>%
...
```{r}
inner_question_answer <- inner_join(questions, answers, by = c("id" = "question_id"), suffix = c("_question", "_answer")) %>%
  mutate(gap = difftime(creation_date_answer, creation_date_question, units = "days"))

inner_question_answer
```
...
```{r}
inner_join(question_answer_counts, question_tags, by = c("id" = "question_id")) %>%
